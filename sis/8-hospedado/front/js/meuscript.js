$(function() { // quando o documento estiver pronto/carregado
    ...
    // em princípio, não foi inserido nenhum exame realizado
    ...
    $(document).on("click", ".excluir_pessoa", function() {
        ...
        $.ajax({
            url: urlX+'/excluir_pessoa/'+id_pessoa,
    ...
    function exibir_exames_realizados() {
        $.ajax({
            url: urlX+'/listar_exames_realizados',
            method: 'GET',
            dataType: 'json', // os dados são recebidos no formato json
            success: listar, // chama a função listar para processar o resultado
            error: function(problema) {
    ...
    $('#modalIncluirExameRealizado').on('shown.bs.modal', function (e) {
     ...
     $(document).on("click", "#btIncluirExameRealizado", function() {
        ...
        // fazer requisição para o back-end
        $.ajax({
            url: urlX+'/incluir_exame_realizado',
            type: 'POST',
            dataType: 'json', // os dados são recebidos no formato json
            contentType: 'application/json', // tipo dos dados enviados
            data: dados, // estes são os dados enviados
        ...
        function dadosIncluidos (retorno) {
            if (retorno.resultado == "ok") { // a operação deu certo?
                ...
                $("#campoPessoaId").val("");
                $("#campoExameId").val("");
                // informar que foi inserido exame realizado
                inserido_exrel = true;
            } else {
                // informar mensagem de erro
                alert(retorno.resultado + ":" + retorno.detalhes);
    ...
    // código a ser executado quando a janela de 
    // inclusão de exames realizados for fechada
    $('#modalIncluirExameRealizado').on('hide.bs.modal', function (e) {
        // se foi inserido algum exame realizado...
        if (inserido_exrel) {
            // atualizar a página de listagem
            exibir_exames_realizados();
            // retorna flag ao valor padrão
            inserido_exrel = false;
        }
    });
